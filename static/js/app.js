document.addEventListener("DOMContentLoaded", () => {
  ...
  async function init() {
    ...
    renderCombinedView(
      state.globalMainData,
      state.globalPoliceData,
      state.globalStats,
    );
  }
  ...
  function setupTabs() {
    ...
    elements.tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        if (targetId === "combined") {
          ...
          if (state.charts["crimeTypeChart"])
            state.charts["crimeTypeChart"].resize();
        }
      });
    });
    ...
    if (navLinks.length >= 3) {
      ...
      alertLink.addEventListener("click", () => {
        ...
        setTimeout(() => {
          if (window.nepalMap) window.nepalMap.invalidateSize();
        }, 100);
      });
    }
  }
  ...
  function initMap() {
    ...
    window.nepalMap = L.map("nepalMap").setView([28.3949, 84.124], 7); // Center of Nepal
          ...
          onEachFeature: function (feature, layer) {
            layer.on({
              ...
              click: async function (e) {
                ...
                layer
                  .bindPopup(
                    `<div style="min-width:150px; text-align:center;"><strong>${districtName}</strong><br><span style="color:#64748b;">Analysing Risk...</span></div>`,
                  )
                  .openPopup();
                ...
                const crimesListEl =
                  document.getElementById("alertRisingCrimes");
                ...
                try {
                  ...
                  const res = await fetch(
                    `/api/predict?year=2082&province=&district=${encodeURIComponent(districtName)}`,
                  );
                  const data = await res.json();

                  if (data.status === "error") {
                    riskTextEl.textContent =
                      "Data unavailable for this district.";
                    return;
                  }
                  ...
                  if (
                    data.safety_status.includes("Rising") ||
                    data.safety_status.includes("High")
                  ) {
                    statusClass = "badge-high";
                    statusLabel = "High Risk";
                  } else if (
                    data.safety_status.includes("Caution") ||
                    data.safety_status.includes("Mid")
                  ) {
                  ...
                  layer.setPopupContent(
                    `<div style="min-width:150px; text-align:center;"><strong>${districtName}</strong><br><span class="${statusClass}" style="color:white; padding:2px 6px; border-radius:4px; font-size:0.8em;">${statusLabel}</span></div>`,
                  );
                ...
                const stationsBody =
                  document.getElementById("policeStationsBody");
                const alertBtn = document.getElementById("emergencyAlertBtn");

                stationsBody.innerHTML =
                  '<tr><td colspan="2" style="padding:1rem; text-align:center; color:#94a3b8;">Loading directories...</td></tr>';
                alertBtn.style.display = "none";

                try {
                  const resPol = await fetch(
                    `/api/stations?district=${encodeURIComponent(districtName)}`,
                  );
                  ...
                  if (stations && stations.length > 0) {
                    ...
                    stationsBody.innerHTML =
                      '<tr><td colspan="3" style="padding:1rem; text-align:center; color:#94a3b8;">No contact information available for this district.</td></tr>';
                  }
                } catch (e) {
                  console.error("Stations fetch failed:", e);
                  stationsBody.innerHTML =
                    '<tr><td colspan="3" style="padding:1rem; text-align:center; color:var(--danger-color);">Error loading contacts.</td></tr>';
                }
              },
            });
          },
  ...
  function setupFilters() {
    // --- Main Crime Filters ---
    elements.mainYear.addEventListener(
      "change",
      (e) => (state.filters.main.year = e.target.value),
    );
    ...
    elements.mainDistrict.addEventListener(
      "change",
      (e) => (state.filters.main.district = e.target.value.trim()),
    );
    ...
    elements.demoYear.addEventListener(
      "change",
      (e) => (state.filters.demo.year = e.target.value),
    );
    elements.demoProvince.addEventListener(
      "change",
      (e) => (state.filters.demo.province = e.target.value),
    );
    ...
    // --- Police Filters ---
    elements.polYear.addEventListener(
      "change",
      (e) => (state.filters.police.year = e.target.value),
    );
    elements.polProvince.addEventListener(
      "change",
      (e) => (state.filters.police.province = e.target.value),
    );
    ...
    // --- Prediction Filters ---
    elements.predYear.addEventListener(
      "change",
      (e) => (state.filters.prediction.year = e.target.value),
    );
    elements.predProvince.addEventListener("change", async (e) => {
      ...
      await updateDistricts(
        ...
        elements.predDistrict,
      );
    });
    elements.predDistrict.addEventListener(
      "change",
      (e) => (state.filters.prediction.district = e.target.value),
    );
    ...
    window.resetPredFilters = () => {
      ...
      state.filters.prediction.province = "";
      elements.predDistrict.innerHTML =
  ...
  async function fetchMainDataWithFilters() {
    ...
    if (state.filters.main.province)
      params.append("province", state.filters.main.province);
    if (state.filters.main.district)
      params.append("district", state.filters.main.district);
  ...
  function renderCombinedView(mainData, polData, stats) {
    ...
    // 1. Header Stats
    if (stats) {
      elements.combTotalCases.textContent = (
        stats.total_crime_cases || 0
      ).toLocaleString();
      elements.combClearanceRate.textContent =
        (stats.clearance_rate || 0) + "%";
      elements.combTopCrime.textContent = stats.top_crime_type || "-";
      elements.combActiveCases.textContent = (
        stats.opened_cases || 0
      ).toLocaleString();
    }
    ...
    normalizedMain.forEach((d) => {
      if (d.year && d.province) {
        ...
        provMap[d.province][d.year] =
          (provMap[d.province][d.year] || 0) + d.cases;
      }
    });
    ...
    if (sortedYears.length > 0) {
      ...
      for (let i = 0; i < sortedYears.length; i++) {
        ...
        let totalCur = 0,
          totalPrev = 0;
        let maxIncreaseProv = null,
          maxIncreaseVal = -Infinity;
        let maxDecreaseProv = null,
          maxDecreaseVal = 0;
        let highestProv = null,
          highestVal = -Infinity;
        ...
        provinces.forEach((p) => {
          ...
          if (prevYear) {
            ...
            totalPrev += prev;
            const d = c - prev;
            if (d > 0 && d > maxIncreaseVal) {
              maxIncreaseVal = d;
              maxIncreaseProv = p;
            }
            if (d < 0 && Math.abs(d) > maxDecreaseVal) {
              maxDecreaseVal = Math.abs(d);
              maxDecreaseProv = p;
            }
          }
        });
        ...
        // Robust check:
        const yearLabel =
          String(curYear).length > 4 ? String(curYear).slice(2) : curYear;
        ...
        if (prevYear) {
          ...
          const totalPct =
            totalPrev > 0 ? ((totalDiff / totalPrev) * 100).toFixed(1) : 0;
          const natDirection = totalDiff >= 0 ? "increased" : "decreased";
          const arrow = totalDiff >= 0 ? "↑" : "↓";
          const natColor =
            totalDiff >= 0 ? "var(--danger-color)" : "var(--success-color)";
  ...
  function renderMainView() {
    ...
    if (state.mainData) {
      state.mainData.forEach(
        (d) =>
          (crimeCounts[d.crime_type] =
            (crimeCounts[d.crime_type] || 0) + d.total_cases),
      );
    }
    ...
    if (entries.length > 0) {
      ...
      const least =
        nonZero.length > 0 ? nonZero[nonZero.length - 1] : ["None", 0];
      ...
      // Clear chart
      if (state.charts["crimeTypeChart"])
        state.charts["crimeTypeChart"].destroy();
    }
  }
  ...
  function renderDemographicsView(data) {
    ...
    data.forEach((d) => {
      ...
      [
        "Victim_Edu_Illiterate",
        "Victim_Edu_Literate",
        "Victim_Edu_SchoolLevel",
        "Victim_Edu_Plus2",
        "Victim_Edu_BachelorPlus",
      ].forEach((k) => {
        edu[k.replace("Victim_Edu_", "")] =
          (edu[k.replace("Victim_Edu_", "")] || 0) + (d[k] || 0);
      });
      [
        "Relationship_Family",
        ...
        rel[k.replace("Relationship_", "")] =
          (rel[k.replace("Relationship_", "")] || 0) + (d[k] || 0);
      });
      [
        "Victim_Age_Group_lte_10",
        "Victim_Age_Group_11_14",
        "Victim_Age_Group_15_16",
        "Victim_Age_Group_17_18",
        "Victim_Age_Group_19_25",
        "Victim_Age_Group_26_35",
        "Victim_Age_Group_36_45",
        "Victim_Age_Group_46_59",
        "Victim_Age_Group_60_plus",
      ].forEach((k) => {
        age[k.replace("Victim_Age_Group_", "")] =
          (age[k.replace("Victim_Age_Group_", "")] || 0) + (d[k] || 0);
      });
    });

    const totalGender = f + m + u;
    elements.demFemaleCount.textContent = f;
    elements.demFemalePct.textContent = totalGender
      ? ((f / totalGender) * 100).toFixed(1) + "%"
      : "0%";
    elements.demMaleCount.textContent = m;
    elements.demMalePct.textContent = totalGender
      ? ((m / totalGender) * 100).toFixed(1) + "%"
      : "0%";
    elements.demUnknownCount.textContent = u;
    elements.demUnknownPct.textContent = totalGender
      ? ((u / totalGender) * 100).toFixed(1) + "%"
      : "0%";

    elements.eduStatsContainer.innerHTML = "";
    Object.entries(edu).forEach(
      ([k, v]) => (elements.eduStatsContainer.innerHTML += createStatRow(k, v)),
    );
    createChart(
      "eduChart",
      "bar",
      ...
      Object.values(edu),
      "Victim Education",
      generateColors(Object.keys(edu).length),
    );

    elements.relStatsContainer.innerHTML = "";
    Object.entries(rel).forEach(
      ([k, v]) => (elements.relStatsContainer.innerHTML += createStatRow(k, v)),
    );
    createChart(
      "relChart",
      "doughnut",
      ...
      Object.values(rel),
      "Relationship",
      generateColors(Object.keys(rel).length),
    );

    elements.ageStatsContainer.innerHTML = "";
    const ageLabels = Object.keys(age).map((k) =>
      k.replace("lte_", "≤").replace("_plus", "+").replace(/_/g, "-"),
    );
    Object.entries(age).forEach(
      ([k, v]) =>
        (elements.ageStatsContainer.innerHTML += createStatRow(
          k.replace("lte_", "≤").replace("_plus", "+").replace(/_/g, "-"),
          v,
        )),
    );
    createChart(
      ...
      Object.values(age),
      "Age Distribution",
      generateColors(ageLabels.length),
    );

    let knownSum = 0,
      count = 0;
    data.forEach((d) => {
      if (d.Known_Offender_Percent != undefined) {
        knownSum += d.Known_Offender_Percent;
        count++;
      }
    });
    elements.knownOffenderVal.textContent = count
      ? (knownSum / count).toFixed(1) + "%"
      : "0%";
  }
  ...
  function renderPoliceView(data) {
    ...
    elements.polClosedPct.textContent =
      opened > 0 ? ((closed / opened) * 100).toFixed(1) + "% Clearance" : "0%";
    createChart(
      "openedClosedChart",
      "doughnut",
      ["Closed", "Pending/Active"],
      [closed, Math.max(0, opened - closed)],
      "Cases",
      ["#10b981", "#f59e0b"],
    );
  }
  ...
  function resetDemographicsUI() {
    ...
    const msg =
      '<div class="no-data-msg" style="padding:1rem; color:#64748b; text-align:center;">No demographics data available for this selection.</div>';
        ...
        options: {
          ...
          plugins: {
            legend:
              type === "bar"
                ? { display: false }
                : { position: "bottom", labels: { color: "#94a3b8" } },
          },
          scales:
            type === "bar"
              ? {
  ...
  function renderPredictionView(data) {
    ...
    elements.predTotalCount.textContent =
      data.total_predicted_cases.toLocaleString();
    ...
    if (data.rising_crimes && data.rising_crimes.length > 0) {
      ...
      elements.risingCrimesList.innerHTML =
        '<div style="color:#94a3b8; font-style:italic;">No crimes predicted to rise significantly.</div>';
    }

    // Chart
    const labels = data.detailed_predictions
      .slice(0, 10)
      .map((d) => d.crime_type);
    const values = data.detailed_predictions
      .slice(0, 10)
      .map((d) => d.predicted_cases);
    createChart(
      "predictionChart",
      "bar",
      labels,
  ...
  async function fetchPoliceNotifications() {
    ...
    try {
      ...
      if (alerts.length > 0) {
        ...
        container.innerHTML =
          '<div style="padding:1rem; color:#64748b;">No new alerts.</div>';
      }
    } catch (e) {
      console.error(e);
    }
  ...
  function renderAlerts(alerts) {
    ...
    containers.forEach((container) => {
      ...
      if (!alerts || alerts.length === 0) {
        container.innerHTML =
          '<div style="padding:1rem; color:#64748b; font-style:italic;">No active emergency alerts.</div>';
        return;
      }
  ...
  async function fetchPoliceComplaints() {
    ...
    try {
      ...
      if (complaints.length > 0) {
        ...
        tbody.innerHTML =
          '<tr><td colspan="4" style="padding:1rem; text-align:center; color:#64748b;">No complaints found.</td></tr>';
      }
    } catch (e) {
      console.error(e);
      tbody.innerHTML =
        '<tr><td colspan="6" style="padding:1rem; text-align:center; color:var(--danger-color);">Error loading complaints.</td></tr>';
    }
  }
  ...
  window.viewFIRDetail = (fir) => {
    ...
    if (!displayDate && fir.created_at) {
      try {
        const Converter =
          window.NepaliDateConverter || window.NepaliDate || window.default;
        if (Converter)
          displayDate = new Converter(new Date(fir.created_at)).format(
            "YYYY-MM-DD",
          );
      } catch (e) {}
      if (!displayDate)
        displayDate = new Date(fir.created_at).toLocaleDateString() + " (AD)";
    }
  ...
  window.openFIRModal = (stationName, districtName) => {
    ...
    if (state.globalMainData) {
      const match = state.globalMainData.find(
        (row) =>
          (row.District &&
  ...
  window.handleEmergencyAlert = async () => {
    ...
    if (!district) {
      ...
      if (
        mapAlertName &&
        mapAlertContainer &&
        mapAlertContainer.style.display !== "none"
      ) {
    ...
    if (!district || district === "District Name") {
      alert(
        "Please select a Police Station or Click a District on the Map first.",
      );
      return;
    }
    ...
    if (
      ...
      try {
        const res = await fetch("/api/create_alert", {
          ...
          body: JSON.stringify({
            district: district,
            message: `Emergency Reported in ${district}`,
          }),
        });
        const result = await res.json();
        ...
        if (result.status === "success") {
          ...
          alertBtns.forEach((btn) => {
            btn.innerHTML =
              '<i class="fas fa-spinner fa-spin"></i> Alert Active - Waiting...';
  ...
  window.handleFIRSubmit = async (event) => {
    ...
    try {
      ...
      if (result.status === "success") {
        alert(
          "Complaint filed successfully! Reference ID: " +
  ...
  async function loadAdminDashboard() {
    ...
    // Populate Admin Crime Year Filter
    const adminYearFilter = document.getElementById("adminCrimeYearFilter");
    if (adminYearFilter && state.allYears) {
      populateDropdowns(adminYearFilter, state.allYears);
      adminYearFilter.onchange = fetchAdminCrime;
    }
    // Default to users sub-tab
    switchAdminSubTab("users-mgmt");

    // Setup admin sub-tab switching
    document.querySelectorAll("[data-admin-tab]").forEach((tab) => {
      tab.onclick = () => switchAdminSubTab(tab.dataset.adminTab);
    });
  }
  ...
  function switchAdminSubTab(targetId) {
    ...
    const activeTab = document.querySelector(`[data-admin-tab="${targetId}"]`);
    if (activeTab) activeTab.classList.add("active");

    document
      .querySelectorAll(".admin-sub-pane")
      .forEach((p) => (p.style.display = "none"));
    const targetPane = document.getElementById(targetId);
    if (targetPane) targetPane.style.display = "block";

    if (targetId === "users-mgmt") fetchAdminUsers();
    if (targetId === "firs-mgmt") fetchAdminFirs();
    if (targetId === "crime-mgmt") fetchAdminCrime();
    if (targetId === "stations-mgmt") fetchAdminStations();
  }
    ...
    try {
      ...
      grid.innerHTML = `
                <div class="stat-card" style="height:auto;">
                    <div class="stat-title">Total FIRs</div>
                    <div class="stat-value" style="font-size:1.5rem;">${data.total_firs}</div>
                </div>
                <div class="stat-card" style="height:auto;">
                    <div class="stat-title">Crime Records</div>
                    <div class="stat-value" style="font-size:1.5rem;">${data.total_crime_records}</div>
                </div>
                ${data.user_stats
                  .map(
                    (s) => `
                    <div class="stat-card" style="height:auto;">
                        <div class="stat-title">${s.role.toUpperCase()} Users</div>
        ...
        .map(
          (u) => `
                <tr style="border-bottom:1px solid #e2e8f0;">
                    ...
                    <td style="padding:0.75rem;">
                        <button onclick='openAdminUserModal(${JSON.stringify(u)})' class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--secondary-color); font-size:0.8rem;">Edit</button>
                        <button onclick="deleteAdminUser(${u.id})" class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--danger-color); font-size:0.8rem;">Delete</button>
                    </td>
                </tr>
            `,
        )
  ...
  async function fetchAdminFirs() {
    const tbody = document.getElementById("adminFirsBody");
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="4">Loading...</td></tr>';
    try {
      const res = await fetch("/api/admin/firs");
      const firs = await res.json();
      tbody.innerHTML = firs
        .map(
          (f) => `
                <tr style="border-bottom:1px solid #e2e8f0;">
                    <td style="padding:0.75rem;">${f.full_name}</td>
                    <td style="padding:0.75rem;">${f.crime_type}</td>
                    <td style="padding:0.75rem;">${f.status}</td>
                    <td style="padding:0.75rem;">
                        <button onclick='viewFIRDetail(${JSON.stringify(f).replace(/'/g, "&#39;")})' class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; font-size:0.8rem;">View</button>
                        <button onclick="deleteAdminFir(${f.id})" class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--danger-color); font-size:0.8rem;">Delete</button>
                    </td>
                </tr>
            `,
        )
  ...
  async function fetchAdminCrime() {
    const tbody = document.getElementById("adminCrimeBody");
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="5">Loading...</td></tr>';

    const yearFilter = document.getElementById("adminCrimeYearFilter");
    const year = yearFilter ? yearFilter.value : "";
    const url = year
      ? `/api/admin/crime?year=${encodeURIComponent(year)}`
      : "/api/admin/crime";

    try {
      const res = await fetch(url);
      const crime = await res.json();
      tbody.innerHTML = crime
        .map(
          (c) => `
                <tr style="border-bottom:1px solid #e2e8f0;">
                    <td style="padding:0.75rem;">${c.year}</td>
                    <td style="padding:0.75rem;">${c.district}</td>
                    <td style="padding:0.75rem;">${c.crime_type}</td>
                    <td style="padding:0.75rem;">${c.total_cases}</td>
                    <td style="padding:0.75rem;">
                        <button onclick='openEditCrimeModal(${JSON.stringify(c)})' class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--secondary-color); font-size:0.8rem;">Edit</button>
                        <button onclick="deleteAdminCrime(${c.id})" class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--danger-color); font-size:0.8rem;">Delete</button>
                    </td>
                </tr>
            `,
        )
  ...
  async function fetchAdminStations() {
    const tbody = document.getElementById("adminStationsBody");
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="4">Loading...</td></tr>';
    try {
      ...
      tbody.innerHTML = stations
        .map(
          (s) => `
                <tr style="border-bottom:1px solid #e2e8f0;">
                    <td style="padding:0.75rem;">${s.district}</td>
                    <td style="padding:0.75rem;">${s.station_name}</td>
                    <td style="padding:0.75rem;">${s.phone}</td>
                    <td style="padding:0.75rem;">
                        <button onclick='openEditStationModal(${JSON.stringify(s).replace(/'/g, "&#39;")})' class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--secondary-color); font-size:0.8rem;">Edit</button>
                        <button onclick="deleteAdminStation(${s.id})" class="btn-primary" style="width:auto; margin:0; padding:0.3rem 0.6rem; background:var(--danger-color); font-size:0.8rem;">Delete</button>
                    </td>
                </tr>
            `,
        )
  ...
  // Modal Handlers
  window.openAddStationModal = () => {
    const modal = document.getElementById("adminStationModal");
    const form = document.getElementById("adminStationForm");
    document.getElementById("adminStationModalTitle").textContent =
      "Add Police Station";
    form.reset();
  ...
  window.openEditStationModal = (s) => {
    const modal = document.getElementById("adminStationModal");
    const form = document.getElementById("adminStationForm");
    document.getElementById("adminStationModalTitle").textContent =
      "Edit Police Station";
    form.station_id.value = s.id;
    form.district.value = s.district;
    form.station_name.value = s.station_name;
    form.phone.value = s.phone;
    modal.style.display = "flex";
  };

  document.getElementById("adminStationForm").onsubmit = async (e) => {
    e.preventDefault();
    const formData = new FormData(e.target);
    const data = Object.fromEntries(formData.entries());
    const id = data.station_id;
    const url = id ? `/api/admin/stations/${id}` : "/api/admin/stations";
    ...
    try {
      ...
      if (res.ok) {
        alert(id ? "Station updated" : "Station added");
        closeAdminModals();
        fetchAdminStations();
      }
    } catch (e) {
  ...
  window.openAdminUserModal = (user) => {
    const modal = document.getElementById("adminUserModal");
    const form = document.getElementById("adminUserForm");
    form.user_id.value = user.id;
    form.full_name.value = user.full_name;
    form.username.value = user.username;
    form.password.value = ""; // Always clear password field on open
    form.role.value = user.role;
    form.district.value = user.district || "";
    form.police_station.value = user.police_station || "";
    modal.style.display = "flex";
  };

  window.openAddCrimeModal = () => {
    const modal = document.getElementById("adminCrimeModal");
    const form = document.getElementById("adminCrimeForm");
    document.getElementById("adminCrimeModalTitle").textContent =
      "Add Crime Record";
    form.reset();
    form.crime_id.value = "";
    modal.style.display = "flex";
  ...
  window.openEditCrimeModal = (c) => {
    const modal = document.getElementById("adminCrimeModal");
    const form = document.getElementById("adminCrimeForm");
    document.getElementById("adminCrimeModalTitle").textContent =
      "Edit Crime Record";
    form.crime_id.value = c.id;
    form.year.value = c.year;
    form.province.value = c.province;
    form.district.value = c.district;
    form.crime_type.value = c.crime_type;
    form.total_cases.value = c.total_cases;
    modal.style.display = "flex";
  };

  window.closeAdminModals = () => {
    document.getElementById("adminUserModal").style.display = "none";
    document.getElementById("adminCrimeModal").style.display = "none";
    document.getElementById("adminStationModal").style.display = "none";
  };
  ...
  document.getElementById("adminCrimeForm").onsubmit = async (e) => {
    e.preventDefault();
    const formData = new FormData(e.target);
    const data = Object.fromEntries(formData.entries());
    const id = data.crime_id;
    const method = id ? "PUT" : "POST";
    const url = id ? `/api/admin/crime/${id}` : "/api/admin/crime";
    try {
      const res = await fetch(url, {
        method: method,
        headers: { "Content-Type": "application/json" },
  ...
  window.deleteAdminFir = async (id) => {
    if (!confirm("Delete FIR?")) return;
    try {
      const res = await fetch(`/api/admin/firs/${id}`, { method: "DELETE" });
      if (res.ok) {
        fetchAdminFirs();
        fetchAdminStats();
      }
  ...
  window.deleteAdminCrime = async (id) => {
    if (!confirm("Delete record?")) return;
    try {
      const res = await fetch(`/api/admin/crime/${id}`, { method: "DELETE" });
      if (res.ok) {
        fetchAdminCrime();
        fetchAdminStats();
      }
